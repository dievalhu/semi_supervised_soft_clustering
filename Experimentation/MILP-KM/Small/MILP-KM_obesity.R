...
dataset <- read.csv("C:/Users/emont/OneDrive/Escritorio/Tesis/Comparacion/Experimentation DATA 2025/Datasets/Small/Obesity.csv", stringsAsFactors = FALSE)

X <- dataset[, c("Age", "Height", "Weight", "FCVC", "NCP", "CH2O", "FAF", "TUE")]
y <- dataset$NObeyesdad     
target_card <- c(272, 287, 351, 297, 324, 290, 290)
...
if (sum(target_card) != nrow(X)) {
  stop(sprintf("sum(target_card) = %d but nrow(X) = %d. Adjust target_card.",
               sum(target_card), nrow(X)))
}

cat(sprintf("Dataset loaded | n=%d | p=%d | k=%d | cardinality: %s\n",
...
solve_milp_assignment <- function(data, centroids, size_constraints) {
  ...
  if (result$status != 0) stop("MILP couldn't find any feasible solution.")
...
clustering_with_size_constraints <- function(data, size_constraints,
  ...
  cat(sprintf("Convergence in %d iteration(s).\n", iteration))
...
cat(sprintf("AMI                                : %.4f\n", ami))
cat(sprintf("NMI                                : %.4f\n", nmi))
cat(sprintf("Execution Time (seg)               : %.2f\n",  algo_time))
cat("--------------------------------------------------\n")
cat("\nReal Cardinality:\n");    print(table(y))
